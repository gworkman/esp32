defmodule Esp32.Image do
  ...
  def parse(<<@magic, segments_count, flash_mode, size_freq, entry_point::little-32, rest::binary>>) do
  ...
  defp parse_extended_header(<<wp_pin, _drive::binary-size(3), chip_id::little-16, _min_rev::little-16, _max_rev::little-16, _res::binary-size(5), hash_appended, rest::binary>>, metadata) do
    metadata_ext = Map.merge(metadata, %{
      wp_pin: wp_pin,
  ...
  defp parse_segments(<<offset::little-32, size::little-32, data::binary-size(size), rest::binary>>, count, acc) do
  ...
  def verify_checksum(%{segments: segments}, expected_checksum) do
    checksum = Enum.reduce(segments, 0xEF, fn segment, acc ->
